        ...
        <main className="pt-16 overflow-hidden">
                    ...
                    <div data-aos='fade-up' className="flex w-full lg:w-1/2 justify-center items-center dark:bg-slate-800">
                        <div className="mx-auto flex justify-center px-4 sm:mt-6 sm:px-4 md:mt-4 lg:mt-2 lg:px-8">
                            <div className="text-center py-10">
                                ...
                                <p className="text-sm text-white font-normal">{MLAIServicesData?.para}</p>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center justify-center w-full lg:h-[40rem] lg:w-1/2 md:mb-0 mb-2">
                        <Image className="object-cover w-full lg:h-[40rem] rounded-md" src={ALML3} alt="glasses photo" width={1000} height={1000} />
                    </div>
                </div>
            </section>
            ...
            <section className="py-10 md:py-20 mt-4 md:mt-0 flex items-center justify-center flex-col">
                <div className="flex w-[90%] flex-col items-center text-start">
                    <p data-aos='fade-right' className="text-2xl md:text-3xl text-blue-700 decoration-orange font-bold capitalize text-center">
                ...
                <div data-aos="zoom-out" className="w-[90%] grid items-center justify-center gap-4 md:gap-10 sm:grid-cols-2 lg:grid-cols-3 my-4 md:my-10">
                    {MLAIServicesData?.features.map((feat: Feature) => (
                        ...
                        >
                            <div className="flex flex-col justify-center items-center rounded-md p-1">
                                <div className="text-8xl text-orange">{React.createElement(feat.icon)}</div>
                                <div className="md:space-y-2 flex flex-col justify-center w-[16rem] sm:w-full">
                                    <h3 className="text-xl md:text-2xl text-center text-blue-800 font-semibold">{feat.title}</h3>
                    ...
                    {currentFeat && (
                        ...
                        >
                            <h2 className="text-xl md:text-2xl text-center text-blue-800 font-semibold">{currentFeat.title}</h2>
                            <p className="text-start font-medium text-blue1 capitalize">{currentFeat?.subTitle}</p>
                            <ul>
                                {currentFeat.list.map((item) => (
                                    <li className="text-sm" key={item.title}>
                                        <span className="font-medium text-md">{item.title}</span>:
                                        <span className="text-sm text-center md:text-md text-muted-foreground">{item.description}</span>
                                    </li>
                                ))}
                            </ul>
                        </Modal>
            ...
            <section>
                <div className='relative w-full h-[40rem] sm:h-[26rem] flex items-center justify-center'>
                    <Image className='object-cover w-full h-[40rem] sm:h-[26rem]' src={background} alt="background image" height={1000} width={1000} />
                ...
                <div className='flex flex-col items-center justify-center'>
                    ...
                    <div className='md:w-[90%] grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-10 px-6'>
                        {MLAIServicesData?.industries.map((feat: Industry) => (
                            <div className='grid grid-cols-1 border-2 p-6 cursor-pointer hover:bg-slate-200 transition-all hover:scale-105 ease-linear' key={feat.id}>
                                <div className='text-6xl text-blue-600'>{React.createElement(feat.icon)}</div>
