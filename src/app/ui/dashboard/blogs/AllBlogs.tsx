                                ...
                                Blogs?.map((blog: Blog) => (
                                    <div key={blog.id} className='max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700'>
                                        <div className='relative'>
                                            <Image src={blog?.image} alt="" width={1000} height={1000} className="rounded-t-md w-full" />
                                            <div><p className='absolute bottom-0 pl-2 pr-4 bg-slate-200 rounded-tr-md'>{formatDate(blog?.postDate)}</p></div>
                                        </div>
                                        <div className='px-4 py-2'>
                                            <h2 className='text-2xl font-bold tracking-tight text-gray-900 dark:text-white'>{blog?.title}</h2>
                                        </div>
