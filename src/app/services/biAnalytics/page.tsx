...
const BiAndAnalyticsDatapage = () => {
  return (
    <main  className="pt-16">
       ...
       {/* First section */}
       <section id="bisection1"  className="overflow-x-hidden">
                    ...
                    >
                        <div className="flex flex-col h-full justify-center items-center gap-4">
                            <h3 className="md:text-[45px] text-MainHeading font-semibold text-center bg-gradient-to-r to-danger bg-clip-text text-transparent from-purple-800 via-blue-800 to-pink-600">{BiAndAnalyticsData.mainTitle}</h3>
                            <p className="text-center md:px-20 md:text-lg text-base">{BiAndAnalyticsData.subTitle}</p>
                            <Link href='/contact' className="inline-block py-2 px-6 bg-orange hover:bg-amber-500 rounded-xl text-md text-white">Contact Now</Link>
                        </div>
                    </motion.div>
                ...
                {/* Second section */}
                <section id="bisection2" className="w-[100vw] overflow-x-hidden -mt-[70px] pt-16 pb-6 md:pb-16 md:px-10 px-3">
                    <div className="w-full">
                    <div className="w-full flex flex-col md:flex-row items-center justify-center gap-6">
                        <div  data-aos="zoom-in" className="w-full p-4 lg:w-1/2 lg:mb-0">
                            <Image src={BiAndAnalyticsData.secondSectionImage} alt="" className="w-full rounded-3xl" />
                        </div>
                        <div className="w-full lg:w-1/2 md:p-10 p-4">
                            <h2 data-aos="fade-left" className="text-SubHeading font-bold mb-4 text-[28px] text-center md:text-start">{BiAndAnalyticsData.secondTitle}</h2>
                            <p data-aos="fade-right" className="text-paragraph text-center text-[18px] md:text-start">{BiAndAnalyticsData.secondSubTitle}</p>
                        </div>
                    </div>
                    </div>
                                    ...
                                    BiAndAnalyticsData?.faqs?.map((faq) => (
                                        <li key={faq.qid} className="text-left md:mb-10">
                                            <div className="flex flex-row items-start md:mb-5">
                                                ...
                                                <div data-aos="fade-right" className="border-black shadow shadow-indigo-300 border px-5 py-2 rounded-b-3xl mt-8 rounded-tr-3xl flex items-center w-fit">
                                                    <h4 className="text-lg leading-6 font-medium text-violet-900">{faq.question}</h4>
                                                </div>
                                            </div>
